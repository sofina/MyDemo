...
public class ExpandTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    public void setCloseText(CharSequence text) {
        ...
        if (maxLines != -1) {
            ...
            Log.d(TAG, "setCloseText: layout.getLineCount() = "+layout.getLineCount());
            Log.d(TAG, "setCloseText: workingText = "+workingText);
    ...
    private Layout createWorkingLayout(String workingText) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            Log.d(TAG, "setCloseText: getWidth() = "+getWidth());
            return new StaticLayout(workingText, getPaint(), mMaxWidth - getPaddingLeft() - getPaddingRight(),
